...
type HTMLInputProps = Omit<InputHTMLAttributes<HTMLInputElement>, 'value' | 'onChange' | 'readOnly'> // первым параметром передаем от куда мы хотим взять типы, вторым параметром, какие типы мы хотим исключить
...
const Input = memo(({
  ...
  const isCaretVisibale = isFocused && !readonly
    ...
    <div className={classNames(cls.InputWrapper, mods, [className])}>
      ...
      <div className={cls.caretWrapper}>
        <input 
          ...
          {...othetProps}
        />
        {isCaretVisibale && <span className={cls.caret} style={{left: `${caretPosition * 9}px`}}/>}             
      </div>      
    </div>
  )
})
